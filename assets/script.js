  ...
  if (domEl) {
    ...
    if (canvasWidth < offsetWidth && canvasHeight < offsetHeight) {
      do {
        ...
        zoom * canvasWidth < offsetWidth &&
        zoom * canvasHeight < offsetHeight
      );
...
function createAddButtonElement() {
  let btnLeft =
      canvas.width / 2 + (canvas.canvasWidth * canvas.manual_zoom) / 2,
...
function createSubtractButtonElement() {
  let btnLeft =
    ...
    btnTop = canvas.height / 2 + (canvas.canvasHeight * canvas.manual_zoom) / 2;
  let subtractButton =
    '<button id="subtractButton" style="position:absolute;top:' +
    btnTop +
    "px;left:" +
    btnLeft +
    'px;cursor:pointer;width:20px;height:20px;">Add</button>';
  let button = document.createElement("button");
  button.style.position = "absolute";
  button.style.left = `${btnLeft}px`;
  button.style.top = `${btnTop}px`;
  ...
  document.querySelector(".canvas-container").appendChild(button);
  document.querySelector("#subtractButton").addEventListener(
    "click",
    function (e) {
      canvas.canvasWidth -= 1080;
      makeArtboard();
...
function buttonPosition() {
  ...
  (panX = canvas.width / 2 + (canvas.canvasWidth * canvas.manual_zoom) / 2),
    (panY =
      canvas.height / 2 - (canvas.canvasHeight * canvas.manual_zoom) / 2 + 60);
...
function addBgImage(src) {
  const img = new Image();
  img.onload = function () {
    let scaleFactor = canvas.canvasWidth / this.width;
...
function limitRectScaling() {
  ...
  if (
    ...
    croppableImage.left + croppableImage.getScaledWidth()
  ) {
  ...
  if (
    ...
    croppableImage.top + croppableImage.getScaledHeight()
  ) {
...
$("#add-bg-randow-image").on("click", function () {
  addBgImage(`https://picsum.photos/${canvas.width}/${canvas.height}`);
});
